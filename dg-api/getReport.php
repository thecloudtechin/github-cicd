...
//echo $data->start;//'2020-06-13' AND '2021-07-22'


$query = "select *,fcmServerKey,users.token,orders.email,orders.discount,orders.number,orders.name,orders.created_at as 
created_at,orders.id as orderid,orders.order_no as order_no,cast(orders.delivery_charges as decimal(4,1)) as delivery_fee_new, 
orders.status as o_status,orders.updated_at as o_updated ,restaurants.hotel_name,restaurants.address as hotel_address from orders 
INNER JOIN restaurants ON restaurants.id=orders.hotel_id INNER JOIN users ON users.id=orders.user_id where (orders.created_at BETWEEN 
'$data->start' AND '$data->end' OR orders.day BETWEEN  '$data->start' AND '$data->end') AND hotel_id IN (" . $data->hotel_id . ") AND 
orders.status != 0 AND orders.user_id != 55 ORDER BY orders.id ASC";
// echo $query;
...
if (mysqli_num_rows($result) > 0)
{
 ...
 $row_array['hotel_id'] = $row["hotel_id"];
        ...
        $row_array['notes'] = str_replace("’","",$row["note"]);
...
// echo json_encode( utf8_encode( $json_response  ));
